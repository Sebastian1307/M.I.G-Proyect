...
void main( void ) {
    ...
    float c = sin(pos * 400.0) * 0.2 + 0.3; // Reduce la contribución del verde
    c = pow(c, 0.1);
    c *= 0.1; // Reduce la intensidad general del color

    float band_pos = fract(uTime * 0.1) * 3.0 - 1.0;
    c += clamp( (1.0 - abs(band_pos - pos) * 10.0), 0.0, 1.0) * 0.05; // Reduzca la contribución del efecto de banda

    c += distortion * 0.05; // Reduzca la contribución de la distorsión
    // ruido
    c += (noise(gl_FragCoord.xy) - 0.5) * (0.05);

    vec4 pixel = texture2D(uMainSampler, outTexCoord);

    // Aplica el efecto sin el tono verde
    vec4 finalColor = pixel + vec4(0.0, c, 0.0, 1.0);

    // Reduce el componente alfa
    finalColor.a *= 0.7; // Puedes ajustar el valor según lo desees
...
class ScalinePostFX extends Phaser.Renderer.WebGL.Pipelines.PostFXPipeline {
    constructor(game) {
        super({
            game,
            renderTarget: true,
